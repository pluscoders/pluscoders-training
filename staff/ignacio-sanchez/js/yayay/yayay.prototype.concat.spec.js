describe('concat', function () {
    ...
    it('should concatenate the non-yayay and yayay arguments into a new yayay', function () {
        ...
        var newYayay = yayay1.concat('d','e','f',1 ,2 , 3, yayay2, true, false, null, undefined)

        expect(newYayay instanceof Yayay).toBeTrue()
        //expect(newYayay.length).toBe(yayay1.length + yayay2.length + 10)
        expect(newYayay[0]).toBe(yayay1[0])
        expect(newYayay[1]).toBe(yayay1[1])
        expect(newYayay[2]).toBe(yayay1[2])
        expect(newYayay[3]).toBe('d')
        expect(newYayay[4]).toBe('e')
        expect(newYayay[5]).toBe('f')
        expect(newYayay[6]).toBe(1)
        expect(newYayay[7]).toBe(2)
        expect(newYayay[8]).toBe(3)
        expect(newYayay[9]).toBe(yayay2[0])
        expect(newYayay[10]).toBe(yayay2[1])
        expect(newYayay[11]).toBe(yayay2[2])
        expect(newYayay[12]).toBe(true)
        expect(newYayay[13]).toBe(false)
        expect(newYayay[14]).toBe(null)
        expect(newYayay[15]).toBe(undefined)
    })
    ...
    it('should concatenate the non-yayay and yayay arguments into a new yayay', function () {
        ...
        var newYayay = yayay1.concat(1, yayay2, 2, yayay3, 3)
